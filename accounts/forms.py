...
class RegistrationForm(forms.ModelForm):
    '''A form for creating new users. Includes all required fields plus
    repeated password'''
    img = forms.ImageField(label='Upload Picture', widget=forms.FileInput(attrs={'class': 'form-input', 'placeholder': 'Upload Picture'}) )
    name = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-input', 'placeholder': 'Name'}) )
    dob = forms.DateField( widget=forms.DateInput(attrs={'type': 'date', 'class': 'form-input', 'placeholder': 'Date of Birth'}))
    phone = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-input', 'placeholder': 'Mobile Number'}))
    ...
    class Meta:
        model = User
        fields = ['img','name', 'dob', 'phone','email']
