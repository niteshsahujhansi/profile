...
from .forms import RegistrationForm, LoginForm, EditProfile
...
def home(request):
    ...
    if request.method == 'POST':
        ...
        fm = EditProfile(request.POST, request.FILES, instance=pi)
        if fm.is_valid():
            user = fm.save()
    ...
    return render(request, 'index.html', {'form':fm, 'age':age})
